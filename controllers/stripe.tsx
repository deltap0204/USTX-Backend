...
const purshaces = require("../models/card.tsx");
const sgMail = require('@sendgrid/mail');
const Transaction = require("../models/transaction.tsx");
...
exports.stripeBuy = (req, res, next) => {
    request.post(process.env.REACT_APP_TOKEN_BACKEND + "api/users/transferToken",
        ...
        function (error, response, body) {
            ...
            request.post(process.env.REACT_APP_URL + "api/thirdparty2",
                ...
                console.log(error2,response2,body2);
                    if (!error2 && response2.statusCode == 200) {
                        let id = body.result.txnHash;
                        ...
                        AdminModel.findOne({}, {}, {}, function (err, admin) {
                            if (admin == null) {
                                ...
                                admin.BuyAmount = (admin.BuyAmount || 0) + parseFloat(transaction.USTX);
                                admin.save()
                            }
                        });
                        let tokenHistory;
...
exports.stripeSell = (req, res, next) => {
    ...
    //----------------
    let id_hash = (require("crypto").createHash("sha256").update(require("mongoose").Types.ObjectId().toString()));
    let id = id_hash.digest('hex');
    let transaction = new Transaction({
        Unique_ID: id,
        UserId: req.body.UserId,
        Name: req.body.Name,
    ...
    TokenByDate.findOne({}, {}, {sort: {'createdAt': -1}}, function (err, data) {
        if (data != null) {
            if (transaction.createdAt.getYear() != data.createdAt.getYear() ||
                ...
                transaction.createdAt.getDate() != data.createdAt.getDate()) {
                // New Day!
                let tokenHistory = new TokenByDate({
...
exports.getPaymentDatabase = (req, res, next) => {
    Transaction.find({UserId: req.body.UserId}).sort({createdAt: -1}).skip(req.body.LoadedCount).limit(6).exec(function (err, docs) {
        return res.status(200).json(
            {payments: docs}
        );
    })
...
exports.getTotalToken = (req, res, next) => {
    Transaction.find({UserId: req.body.UserId}).exec(function (err, docs) {
        let buy = 0.0;
        let sell = 0.0;
        let total = 0.0;

        for (let i = 0; docs.length > i; i++) {
            let transaction = docs[i];
            if (transaction.type === false) {
                buy += parseFloat(transaction.USTX);
            } else {
                sell += parseFloat(transaction.USTX);
            }
        }
...
function invoice_Email(UserEmail) {
